	   ...
	   <View style={{flex:1,flexDirection:'column',backgroundColor:'#fff',}}> 
                ...
                <View style={styles.card}>
				  ...
				  <View style={{flex:1,alignItems:'center',justifyContent:'center'}}>   
										...
										<Text style={{color:'white',fontSize:16}} allowFontScaling={false}>考勤打卡</Text>
				...
				<View style={{flexDirection:'column',backgroundColor:'#ececec',borderBottomWidth:1,borderColor:'#ececec',}}>
				    <View style={{flexDirection:'row',backgroundColor:'#4385f4',height:100,justifyContent:'space-between',alignItems:'center',paddingLeft:15,paddingRight:15,}}>
					   <View style={{flexDirection:'column',}}>
					      <Text style={{color:'#fff',fontSize:22}} allowFontScaling={false}>{this.state.day}</Text>
						  <Text style={{color:'#fff',fontSize:18,marginTop:5,}} allowFontScaling={false}>{this.state.time}</Text>
					   </View>
					...
					<View style={{flexDirection:'row',height:50,alignItems:'center',paddingLeft:15,backgroundColor:'#fff',}}>
					   ...
					   <View style={{flex:1,marginLeft:15,}}>
					      <Text style={{fontSize:14,}} allowFontScaling={false}>{this.state.map}</Text>
					   </View>
					</View>
				</View>
				<ScrollView style={{flex:1,backgroundColor:'#fff',}}>
					...
					{this.state.times.map((data,i) => {
					    ...
					    <View style={{flexDirection:'column',paddingLeft:25,alignItems:'center',}}>
						   <Text style={{fontSize:14,}} allowFontScaling={false}>第{data.order}次打卡</Text>
						   
						</View>
						<View style={{flexDirection:'column',flex:1,alignItems:'center',}}> 
						   <Text style={{fontSize:14,}} allowFontScaling={false}>{data.qd_time}</Text>   
						</View>
					</View>
					})}
				  ...
				  <TouchableOpacity   
				   ...
				   <View style={{width:100,height:100,borderRadius:50,backgroundColor:'#cdcdcd',alignItems:'center',justifyContent:'center',}}>
				      <View style={{width:90,height:90,borderRadius:50,backgroundColor:'#4385f4',alignItems:'center',justifyContent:'center',}}>
					      <Text style={{color:'#fff',fontSize:20,}} allowFontScaling={false}>打卡</Text>
						  <Text style={{color:'#fff',marginTop:5,fontSize:16,}} allowFontScaling={false}>{this.Gdate(new Date(this.state.nowa).getHours())}:{this.Gdate(new Date(this.state.nowa).getMinutes())}:{this.Gdate(parseInt(this.state.nowa/1000%60))}</Text>
					  </View>   
				   </View> 
				   </TouchableOpacity>
				</View> : <View></View>}   
				{this.state.statu.length>0 ? <Animated.View style={{opacity: this.state.fadeAnim,padding:10,width:200,backgroundColor:'rgba(23, 22, 22, 0.7)',justifyContent:'flex-start',alignItems:'center',position:'absolute',top:(Dimensions.get('window').height-150)/2,left:(Dimensions.get('window').width-200)/2,}}>
				  ...
				  <Text style={{fontSize:16,color:'#fff',marginTop:20,}} allowFontScaling={false}>{this.state.statu}</Text>   
			   ...
			   {this.state.add ? <View style={{position:'absolute',top:60,right:5,flexDirection:'column',width:120,height:100,}}> 
				   <View style={{width:120,height:45,backgroundColor:'#fff',borderRadius:5,flexDirection:'column',alignItems:'center',marginTop:10,}}>
				     <TouchableOpacity  onPress={this._Tj.bind(this)}>
					   <View style={{width:120,alignItems:'center',height:45,flexDirection:'row',paddingLeft:10,}}>
						  ...
						  <Text style={{marginLeft:15,fontSize:16,}} allowFontScaling={false}>统计</Text>
					   </View>
					 </TouchableOpacity> 
                      
				   </View>  		   
				   <Icon name="md-arrow-dropup" style={{position:'absolute',top:-8,right:13,padding:0, backgroundColor:'transparent'}} color="#fff" size={30}  />
			   </View> : <View></View>}
